        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredLands.map((land) => (
              <Card key={land.id} className="border border-yellow-300 bg-yellow-50 rounded-2xl shadow">
                <CardContent className="p-4">
                  <div className="mb-2">
                    <p className="text-sm text-gray-600">ID: {land.id}</p>
                    <h2 className="text-lg font-semibold text-yellow-800">{land.street}, {land.city} - {land.state}</h2>
                    <p className="text-sm text-gray-600">Preço: <span className="font-bold text-green-600">R$ {parseFloat(land.price).toFixed(2)}</span></p>
                  </div>
